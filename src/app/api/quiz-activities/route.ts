...
export async function POST(req: NextRequest) {
  try {
    ...
    const { subProjectId, title, description, autoGenerate, quizDesignTemplateContent, quizDesignTemplateId, analysisPrompt, passScore } = await req.json();
    ...
    // 自动生成题目：必须基于作业设计模板内容（删除"无模板"兜底逻辑）
    if (autoGenerate) {
      if (!quizDesignTemplateContent || !quizDesignTemplateContent.trim()) {
        return new Response("缺少作业设计模板，请先选择作业设计模板", { status: 400 });
      }
        ...
        if (aiResponse.ok) {
          ...
          const quiz = await prisma.quizActivity.create({
            data: {
              ...
              quizDesignTemplateId: quizDesignTemplateId || null,
