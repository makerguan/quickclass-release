...
export async function POST(req: NextRequest) {
  try {
    ...
    const { subProjectId, title, description, autoGenerate, quizDesignTemplateContent, analysisPrompt } = await req.json();
    ...
    if (quizDesignTemplateContent && autoGenerate) {
      ...
      // AI失败时仍创建空作业
      const quiz = await prisma.quizActivity.create({
        data: { subProjectId, title, description: description || null, status: "INACTIVE", updatedAt: new Date() },
        include: { Question: { orderBy: { order: "asc" } } },
      });
